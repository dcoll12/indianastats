...
def render_grassroots_table(df, partisan_data, race_results=None, senate_2022=None, district_geojson=None):
    ...
    for _, contact in df.iterrows():
        ...
        if dtype == 'senate' and not rdata and senate_2022:
            ...
            if s22:
                ...
                race_year = 2022
        if dtype and dnum:
            prefix = DIST_LABELS.get(dtype, dtype.upper())
            group_key = f'{prefix}-{dnum}'
        elif dtype is None:
            group_key = 'Statewide'
        else:
    ...
    def sort_key(r):
        ...
        return (dtype_order.get(r['dtype'], 50), r['dnum'] or 9999)
    ...
    legend_html = (
        ...
        '<i>((R&minus;D)/(R+D))</i>. 2024 uses precinct-level results (VEST dataset) aggregated by district. '
        '2020 uses county-level results apportioned by census block weights. House IN-Index is 2024 only.</div>'
